...
class TestAccountsUI:
    ...
    @patch("finarius_app.ui.accounts.get_db")
    @patch("finarius_app.ui.accounts.st")
    ...
    @patch("finarius_app.ui.accounts.get_all_accounts")
    @patch("finarius_app.ui.accounts.get_db")
    @patch("finarius_app.ui.accounts.st")
    ...
    @patch("finarius_app.ui.accounts.get_all_accounts")
    @patch("finarius_app.ui.accounts.calculate_portfolio_value")
    @patch("finarius_app.ui.accounts.get_db")
    @patch("finarius_app.ui.accounts.st")
    def test_render_accounts_page_with_accounts(self, mock_st, mock_get_db, mock_calc_value, mock_get_all_accounts):
        ...
        with patch("finarius_app.ui.accounts.get_account_by_id", mock_get_account_by_id):
            render_accounts_page()
    ...
    @patch("finarius_app.ui.accounts.get_db")
    @patch("finarius_app.ui.accounts.st")
    @patch("finarius_app.ui.accounts.set_error_message")
    def test_render_accounts_page_form_validation(self, mock_set_error, mock_st, mock_get_db, mock_get_all_accounts):
        ...
        mock_st.selectbox = MagicMock(return_value="USD")
        mock_st.form_submit_button = MagicMock(return_value=True)
        mock_st.rerun = MagicMock()
        
        render_accounts_page()

        # Should call set_error_message for empty name
        assert mock_set_error.called or not mock_st.form_submit_button.return_value

    @patch("finarius_app.ui.accounts.get_all_accounts")
    @patch("finarius_app.ui.accounts.get_account_by_id")
    @patch("finarius_app.core.models.get_transactions_by_account")
    @patch("finarius_app.ui.accounts.calculate_portfolio_value")
    @patch("finarius_app.ui.accounts.get_db")
    @patch("finarius_app.ui.accounts.st")
    @patch("finarius_app.ui.accounts.set_success_message")
